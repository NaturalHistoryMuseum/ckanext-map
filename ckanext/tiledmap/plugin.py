...
from sqlalchemy import Column, MetaData, Numeric, Table, cast, func, not_, or_
...
class TiledMapPlugin(SingletonPlugin):
    ...
    def update_config(self, config):
        ...
        toolkit.add_resource(u'theme/public', u'ckanext-tiledmap')
    ...
    def before_map(self, map):
        ...
        '''
        map.connect('/map-tile/{z}/{x}/{y}.png',
                    controller=u'ckanext.tiledmap.controllers.map:MapController',
                    action=u'tile')
        map.connect('/map-grid/{z}/{x}/{y}.grid.json',
                    controller=u'ckanext.tiledmap.controllers.map:MapController',
                    action=u'grid')
        map.connect('/map-info',
                    controller=u'ckanext.tiledmap.controllers.map:MapController',
                    action=u'map_info')
    ...
    def info(self):
        ...
        return {
            ...
            u'schema': {
                ...
                u'utf_grid_fields': [toolkit.get_validator('ignore_empty'),
                                     self._is_datastore_field],
    ...
    def _is_datastore_field(self, key, data, errors, context):
        ...
        if isinstance(data[key], list):
            if not set(data[key]).issubset(
                    self._get_datastore_fields(context[u'resource'].id, context)):
                raise toolkit.Invalid(
                    u'"{0}" is not a valid parameter'.format(data[key]))
        elif not data[key] in self._get_datastore_fields(context[u'resource'].id,
                                                         context):
    ...
    def _color_validator(self, value, context):
        ...
        if re.match(u'^#?([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$', value):
            ...
            raise toolkit.Invalid(toolkit._(
                u'Colors must be formed of three or six RGB hex value, optionally '
                u'preceded by a # sign (eg. #E55 or #F4A088)'))
    ...
    def _float_01_validator(self, value, context):
        ...
        try:
            ...
            raise toolkit.Invalid(
                toolkit._(u'Must be a decimal number, between 0 and 1'))
    ...
    def _is_view_id(self, value, context):
        ...
        if value:
            views = toolkit.get_action(u'resource_view_list')(context, {
                u'id': context[u'resource'].id
                })
            if value not in [v[u'id'] for v in views]:
                raise toolkit.Invalid(
                    toolkit._(u'Must be a view on the current resource'))
    ...
    def _is_latitude_field(self, value, context):
        ...
        query = query.where(
            or_(cast(table.c[value], Numeric) < -90, cast(table.c[value], Numeric) > 90))
        with db.begin() as connection:
            try:
                query_result = connection.execute(query)
            except DataError as e:
                raise toolkit.Invalid(
                    toolkit._(u'Latitude field must contain numeric data'))

            row = query_result.fetchone()
            query_result.close()
            if row[u'count'] > 0:
                raise toolkit.Invalid(
                    toolkit._(u'Latitude field must be between -90 and 90'))
        return value
    ...
    def _is_longitude_field(self, value, context):
        ...
        query = select([func.count(1).label(u'count')], from_obj=table)
        query = query.where(not_(table.c[value] == None))
        query = query.where(or_(cast(table.c[value], Numeric) < -180,
                                cast(table.c[value], Numeric) > 180))
        with db.begin() as connection:
            try:
                query_result = connection.execute(query)
            except DataError as e:
                raise toolkit.Invalid(
                    toolkit._(u'Longitude field must contain numeric data'))

            row = query_result.fetchone()
            query_result.close()
            if row[u'count'] > 0:
                raise toolkit.Invalid(
                    toolkit._(u'Longitude field must be between -180 and 180'))
        return value
