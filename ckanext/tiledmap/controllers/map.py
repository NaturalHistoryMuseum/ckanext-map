...
class MapController(toolkit.BaseController):
    ...
    def __before__(self, action, **params):
        ...
        if not request.params.get(u'resource_id'):
            toolkit.abort(400, _(u'Missing resource id'))

        self.resource_id = request.params.get(u'resource_id')

        try:
            resource_show = toolkit.get_action(u'resource_show')
            self.resource = resource_show(None, {u'id': self.resource_id})
        except toolkit.NotFound:
            toolkit.abort(404, _(u'Resource not found'))
        except toolkit.NotAuthorized:
            toolkit.abort(401, _(u'Unauthorized to read resources'))
        resource_view_show = toolkit.get_action(u'resource_view_show')
        self.view_id = request.params.get(u'view_id')
        self.view = resource_view_show(None, {u'id': self.view_id})
    ...
    def map_info(self):
        ...
        if not self.view[u'enable_plot_map'] and not self.view[u'enable_grid_map'] and not self.view[u'enable_heat_map']:
            return json.dumps({
                u'geospatial': False,
                u'fetch_id': fetch_id
            })
        ...
        info = toolkit.get_action(u'datastore_query_extent')({}, {
            ...
            u'limit': 1,
            u'q': urllib.unquote(request.params.get(u'q', u'')),
            u'fields': u'_id'
        })
    ...
    def _get_request_filters(self):
        ''' '''
        filters = {}
        for f in urllib.unquote(request.params.get(u'filters', u'')).split(u'|'):
            if f:
                (k, v) = f.split(u':', 1)
                if k not in filters:
                    filters[k] = []
